...
from .forms import Application2017Form
from .helpers import is_valid_email_address
from .models import Airport, Draft, Application2017, Reference
from .resources import Application2017Resource, DraftResource, UserResource, Round0RatingResource, Round1RatingResource, Round2RatingResource
...
def load_json_to_initial(data):
    ...
    # Year 2017: MULTIFIELD_NAMES += ['gender_0', 'ethnicity_0', ]
...
class ApplicationView2017(FormView):
    """View shows the application itself"""
    form_class = Application2017Form
    template_name = 'application/application_form-2017.html'
    ...
    def save_draft(self, form):
        ...
        acknowledgements = form.data.getlist('acknowledgments')
...
class PreFilledApplicationView(ApplicationView2017):
...
class ReferralApplicationView(ApplicationView2017):
...
def check_email(request, email):
    if Application2017.objects.filter(email=email).exists():
        context = {}
...
class ThankYou(TemplateView):
    ...
    def get(self, request, pk, *args, **kwargs):
        self.application = get_object_or_404(Application2017, pk=pk)
        return redirect('http://www.opencon2017.org/thank_you?referral=' + str(self.application.my_referral))
        # return super().get(request, *args, **kwargs)


class Application2017ExportView(View):

    def get(self, request, *args, **kwargs ):
        dataset = Application2017Resource().export()
        response = HttpResponse(dataset.csv, content_type='csv')
        response['Content-Disposition'] = 'attachment; filename=opencon_applications_2017.csv'
        return response
